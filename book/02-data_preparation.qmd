...
df_covars <- terra::extract(
  ...
  ID = TRUE   # To not add a default ID column to the output
) |> 
  as_tibble()  # to have it in a nicely readable format

df_full <- bind_rows(df_obs, df_covars)
...
```{r}
df_full |>
  select(1:20) |> # reduce data for readability of the plot
  visdat::vis_miss()
```
